...
function setHeapSizeLimitMB({db, aggLimit, globalLimit}) {
    assert.commandWorked(
        db.adminCommand({setParameter: 1, internalQueryJavaScriptHeapSizeLimitMB: aggLimit}));
...
function runCommonTests(db) {
    ...
    setHeapSizeLimitMB({db: db, aggLimit: sufficentHeapSizeMB, globalLimit: sufficentHeapSizeMB});
    ...
    // too low.
    setHeapSizeLimitMB({db: db, aggLimit: tooSmallHeapSizeMB, globalLimit: sufficentHeapSizeMB});
    ...
    setHeapSizeLimitMB({db: db, aggLimit: sufficentHeapSizeMB, globalLimit: tooSmallHeapSizeMB});
    assert.commandFailedWithCode(db.runCommand(aggregateWithJSFunction),
                                 ErrorCodes.JSInterpreterFailure);
    assert.commandFailedWithCode(db.runCommand(aggregateWithJSAccumulator),
                                 ErrorCodes.JSInterpreterFailure);
...
function runShardTests(db) {
    ...
    // A find command with JavaScript agg expression is expected to fail when the aggregation
    // specific heap size limit is too low.
    setHeapSizeLimitMB({db: db, aggLimit: tooSmallHeapSizeMB, globalLimit: sufficentHeapSizeMB});
    ...
    // The mapReduce command and $where are not limited by the aggregate command heap size limit and
    // will succeed even if it is set too low.
    setHeapSizeLimitMB({db: db, aggLimit: tooSmallHeapSizeMB, globalLimit: sufficentHeapSizeMB});
    assert.commandWorked(db.runCommand(mapReduce));
    // TODO SERVER-45454: Uncomment when $where is executed via  aggregation JavaScript expression.
    // assert.commandWorked(db.runCommand(findWithWhere));
